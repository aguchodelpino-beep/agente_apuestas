...
def _load_cache_items() -> List[Dict[str, Any]]:
    path = CACHE_DIR / "cachefutbol.json"
    if not path.exists():
        return []
...
def load_futbol_events() -> List[Dict[str, Any]]:
    return _load_cache_items()


def get_futbol_event_by_id(internal_event_id: str) -> Optional[Dict[str, Any]]:
...
def load_futbol_snapshot_history(internal_event_id: str) -> List[Dict[str, Any]]:
...
def get_pick_clv_context(
    ...
    *,
    ...
    if not HISTORY_DB.exists():
        return PickContext()
    snapshots = load_futbol_snapshot_history(internal_event_id)
    if not snapshots:
        return PickContext()
    filtered = [
