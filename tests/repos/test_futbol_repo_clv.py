...
def _prepare_db(db_path: Path) -> None:
    with sqlite3.connect(db_path) as conn:
        ...
        conn.executemany(
            ...
            """,
            [
                ("evt_fut_1","futbol","pinnacle","h2h","home","Real Madrid",2.20,"2026-05-09T10:00:00Z","2026-05-09","f1"),
                ("evt_fut_1","futbol","pinnacle","h2h","home","Real Madrid",2.05,"2026-05-09T14:00:00Z","2026-05-09","f2"),
                ("evt_fut_1","futbol","pinnacle","h2h","away","Barcelona",3.10,"2026-05-09T10:00:00Z","2026-05-09","f3"),
                ("evt_fut_1","futbol","pinnacle","h2h","away","Barcelona",3.30,"2026-05-09T14:00:00Z","2026-05-09","f4"),
            ],
        )


def test_load_futbol_events_reads_cache(tmp_path: Path, monkeypatch):
    cache_dir = tmp_path / "cache_enriched"
    cache_dir.mkdir()
    (cache_dir / "cachefutbol.json").write_text(
        json.dumps({
            "sport": "futbol",
            "updated_at": "2026-05-09T08:00:00Z",
            "window_days": 3,
            "items": [{"internal_event_id": "evt_fut_1", "home_team": "Real Madrid"}],
        }),
        encoding="utf-8",
    )
...
def test_get_pick_clv_context(tmp_path: Path, monkeypatch):
    ...
    assert clv.opening_odds == 2.20
    assert clv.closing_odds == 2.05
    assert clv.clv_pct == pytest.approx(-6.818, abs=1e-2)
