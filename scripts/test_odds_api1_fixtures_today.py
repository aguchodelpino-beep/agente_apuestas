...
URL = "https://odds-api1.p.rapidapi.com/fixtures/today"
HEADERS = {
...
out_file = f"logs/odds_api1_fixtures_today_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}.log"
...
try:
    ...
    write(body[:2000] if body else "<empty body>")
    write("BODY_SAMPLE_END")

    try:
        parsed = resp.json()
        if isinstance(parsed, list):
            write("JSON_TYPE: list")
            write(f"JSON_LEN: {len(parsed)}")
            if parsed:
                write("JSON_FIRST_ITEM:")
                write(json.dumps(parsed[0], ensure_ascii=False)[:1200])
        elif isinstance(parsed, dict):
            write("JSON_TYPE: dict")
            write(f"JSON_KEYS: {list(parsed.keys())[:40]}")
            write("JSON_DICT_SAMPLE:")
            write(json.dumps(parsed, ensure_ascii=False)[:1200])
        else:
